...
1. Heading and paragraph content is captured as `para` entries in source order.
...
1. Slide text is emitted as `para` entries with slide provenance paths.
...
4. Body-level `table` and `graph` elements include `source-path` provenance.
