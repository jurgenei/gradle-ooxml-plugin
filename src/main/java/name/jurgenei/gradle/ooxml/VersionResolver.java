...
final class VersionResolver {
    private static final Pattern DASHED_V_PREFIX_PATTERN = Pattern.compile("^(?<version>v[0-9]+(?:\\.[0-9]+)*)-.+$", Pattern.CASE_INSENSITIVE);
    ...
    static ResolvedVersion resolve(String fileStem) {
        Matcher dashedPrefix = DASHED_V_PREFIX_PATTERN.matcher(fileStem);
        if (dashedPrefix.matches()) {
            return new ResolvedVersion(fileStem, dashedPrefix.group("version").toLowerCase());
        }
